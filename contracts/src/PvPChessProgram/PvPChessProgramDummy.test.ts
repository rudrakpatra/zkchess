...
describe('PvPChessProgramDummy', () => {
  ...
  it('move -> move', async () => {
    ...
    const proof0 = await PvPChessProgramProof.dummy(rollupstate,initialGameState,2);
    // white's move
    const gs1=new GameObject(initialGameState).toUpdated(Move.fromLAN('e2', 'e3'));
    const proof1 = await PvPChessProgramProof.dummy(rollupstate,gs1,2)

    expect(proof1.publicOutput.encode()).toEqual(gs1.encode());
    // black
    const gs2=new GameObject(gs1).toUpdated(Move.fromLAN('d7', 'd5'));
