...
class GameMachine {
	...
	private async repeatUntilValidLocalProof(socket: Socket) {
	...
	public async playAsWhite(socket: Socket) {
		...
		while (true) {
			...
			await this.repeatUntilValidLocalProof(socket);
			//wait for network proof, retry until a valid proof is generated from network
			await this.repeatUntilValidNetworkProof();
		}
	}
	public async playAsBlack(socket: Socket) {
